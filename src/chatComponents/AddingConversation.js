...
export default function AddingConversation(props) {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_API_ROUTE}/chat/users?q=${query}`
        );
  ...
  function handleAddingGroupmembers(user) {
    setAddedMembers((prev) => {
      ...
      if (array.includes(user.idusers)) return [...prev];
  ...
  function handleDeleteGroupMembers(user) {
    setAddedMembers((prev) => {
      ...
      if (array.includes(user.idusers)) {
        ...
        if (indexElement === 0) return prev.shift();
  ...
  async function createConversation() {
    if (
      addingConv.conversation_name === "" ||
      addingConv.group_members_ids.length === 0
    ) {
    ...
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_API_ROUTE}/chat/create_conversation`,
        {
          ...addingConv,
  ...
  return (
    <div className="add-new-conversation">
      ...
      <input
        ...
        value={addingConv.conversation_name}
        onChange={handleAddingConv}
        placeholder="conversation name"
      />
      <AddedGroupMembers
        members={addedMembers}
        handleDelete={handleDeleteGroupMembers}
      />
